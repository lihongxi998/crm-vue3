    ...
    <el-card>
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            ...
            <el-button type="primary" size="small" @click="handleCreate">新建合同</el-button>
          </div>
        </div>
      </template>
      ...
      <el-form :inline="true" :model="queryParams" class="filter-form">
        <el-form-item label="合同编号">
          <el-input v-model="queryParams.contractSn" placeholder="请输入合同编号" clearable />
        </el-form-item>
      ...
      <el-table :data="contracts" v-loading="loading" style="width: 100%" stripe @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="50" />
        <el-table-column prop="contractSn" label="合同编号" width="160" />
...
import { listContract, delContract, listCustomer as listCrmCustomer } from '@/api/crm'
...
const loadData = async () => {
  ...
  try {
    ...
    const res: any = await listContract(params)
...
const resetQuery = () => {
  Object.assign(queryParams, { contractSn: '', customerId: undefined, status: undefined })
  pagination.pageNo = 1
  loadData()
}

const handleCreate = () => { ElMessage.info('新建合同功能开发中...') }
const handleDetail = (row: any) => { ElMessage.info(`查看合同详情: ${row.contractSn}`) }
const handleExport = () => { ElMessage.info('导出功能开发中...') }

const handleAudit = async (row: any) => {
  try {
    await ElMessageBox.confirm(`确定${row.status === 10 ? '审核' : '反审核'}该合同吗？`, '提示', { type: 'warning' })
